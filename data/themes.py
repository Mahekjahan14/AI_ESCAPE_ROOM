...
# Unsplash — space & haunted (works on most networks)
_UNSPLASH = {
    "space_cover": "photo-1446776811953-b23d57bd21aa",
    "space_r1": "photo-1614732414440-30163e4e576d",
    "space_r2": "photo-1454789548928-9efd52dc4031",
    "space_r3": "photo-1464802686167-b939a691aff9",
    "space_r4": "photo-1451187580459-43490279c0fa",
    "space_r5": "photo-1517976487492-5750f3195933",
    "haunted_cover": "photo-1518709268805-4e9042af9f23",
    "haunted_r1": "photo-1509244751260-7d23d2fd7f4f",
    "haunted_r2": "photo-1513885535751-8b9238bdee6b",
    "haunted_r3": "photo-1518709268805-4e9042af9f23",
    "haunted_r4": "photo-1556910103-1c02745aae4d",
    "haunted_r5": "photo-1518331483807-f834fdf25f2f",
}

# Pexels — cyber, egypt, pirate, lab (reliable CDN, thematic photos)
_PEXELS = {
    "cyber_cover": 2599244,
    "cyber_r1": 3861969,
    "cyber_r2": 325229,
...
def _pexels_url(photo_id: int) -> str:
    return (
        ...
        f"?auto=compress&cs=tinysrgb&w=800&h=450&fit=crop"
    )


def _unsplash_url(photo_id: str) -> str:
    return f"https://images.unsplash.com/{photo_id}?auto=format&fit=crop&w=800&h=450&q=80&fm=jpg"


def _img(key: str) -> str:
    """Thematic image — local file if present, else Pexels or Unsplash."""
    local = _ASSETS / f"{key}.jpg"
    if local.is_file() and local.stat().st_size > 3000:
        return str(local)
    if key in _PEXELS:
        return _pexels_url(_PEXELS[key])
    return _unsplash_url(_UNSPLASH[key])
...
THEMES = {
    ...
    "haunted_mansion": {
        ...
        "cover_image": _img("haunted_cover"),
        "rooms": [
            {
                "name": "Creaking Foyer",
                "image": _img("haunted_r1"),
                "description": (
    ...
    "pirate_cove": {
        ...
        "cover_image": _img("pirate_cover"),
        "rooms": [
            {
                "name": "Main Deck",
                "image": _img("pirate_r1"),
                "description": (
